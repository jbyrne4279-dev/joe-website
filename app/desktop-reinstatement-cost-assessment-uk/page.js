        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            {whoNeedsIt.map(({ icon, title, desc }) => (
              <div key={title} className="gold-outline-shine flex gap-5 items-start rounded-2xl p-6" style={{ background: '#dbe6fb' }}>
          ...
          <ul className="grid sm:grid-cols-2 gap-x-12">
            {included.map(({ icon, label, detail }) => (
              <li key={label} tabIndex={0} className="group py-4 border-t border-zinc-200 flex gap-3 items-start cursor-default outline-none">
                <span className="material-symbols-outlined shrink-0 mt-0.5" style={{ fontSize: '1.15rem', color: '#1e40af' }}>{icon}</span>
                <span>
