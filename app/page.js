    ...
    <main>
      ...
      <section className="relative py-12 sm:py-24 overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-8">
          <div className="gold-outline-shine liquid-glass-dark text-center mb-8 sm:mb-16 space-y-3 sm:space-y-4 max-w-2xl mx-auto rounded-2xl sm:rounded-3xl p-6 md:p-14 sovereign-shadow">
            <span className="text-[0.65rem] sm:text-xs font-bold uppercase tracking-widest text-white/70 flex items-center justify-center gap-2"><span className="material-symbols-outlined" style={{fontSize:'1rem'}}>workspace_premium</span>Who we are</span>
            <h1 className="text-2xl sm:text-4xl md:text-5xl font-bold tracking-tight text-white"><span className="text-[#d4af37]">50+ years</span> of Rebuild Cost Assessments in the UK.</h1>
            <p className="text-white/75 text-sm sm:text-lg leading-relaxed max-w-xl mx-auto">We are a RICS-regulated practice based in London’s West End, specialising exclusively in Reinstatement Cost Assessments that help protect property owners from under-insurance and potential shortfalls in their insurance cover.</p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center pt-2 sm:pt-3">
              <Link href="/contact#contact-form" className="btn-shine text-white px-6 py-3 sm:px-8 sm:py-4 rounded-full font-bold text-sm sm:text-base active:scale-[0.97] transition-transform sovereign-shadow text-center inline-flex items-center justify-center gap-2">
                Get Report
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-6">
            {[
              ...
              <div key={title} className="service-pulse-card flex flex-col items-center text-center gap-3 sm:gap-5 p-4 sm:p-8 rounded-2xl bg-white shadow-md hover:shadow-lg transition-all duration-300"
                style={{ animation: `card-pulse-${['left','middle','right'][i]} 9s ease-in-out infinite` }}>
                {title === '50+ Years Experience' ? (
                  <div className="logo-shine w-16 h-16 sm:w-24 sm:h-24 rounded-xl">
                    <Image src={img} alt={title} width={96} height={96} className="w-16 h-16 sm:w-24 sm:h-24 object-contain" />
                  </div>
                ) : (
                  <Image src={img} alt={title} width={96} height={96} className="w-16 h-16 sm:w-24 sm:h-24 object-contain" />
                )}
                <div className="space-y-1 sm:space-y-2">
                  <p className="font-bold text-sm sm:text-base text-primary">{title}</p>
      ...
      <section className="scroll-reveal pt-10 pb-20 px-8 bg-[#0f3d28]" id="which">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12 space-y-3">
            ...
            <h2 className="text-4xl font-bold text-white">Looking for a <span className="text-shine-yellow">Reinstatement Cost Assessment</span> for insurance purposes?</h2>
            ...
            {[
              ...
              >
                <div className="flex flex-col items-center gap-3">
                  <Image src={img} alt={`${name} reinstatement cost assessment London`} width={96} height={96} className="w-24 h-24 object-contain" />
      ...
      <section className="scroll-reveal pt-10 pb-20 px-8 md:px-12 bg-[#0f3d28]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 max-w-5xl mx-auto">
          {[
            ...
            { icon: 'shield', title: 'Professional Indemnity', desc: 'All assessments are backed by full professional indemnity insurance, protecting your property and your position in the event of an insurance claim.' },
